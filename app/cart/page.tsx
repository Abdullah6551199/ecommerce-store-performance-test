    ...
    <div className="min-h-screen bg-[#070d09] text-white py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-xs text-white/50 mb-6">
          ...
          <span>/</span>
          <span className="text-[#18C729]">Shopping Cart</span>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-6 mb-8">
          ...
          <Link
            href="/"
            className="text-xs text-[#18C729] hover:underline flex items-center gap-1 font-semibold"
          >
        ...
        {items.length === 0 ? (
          /* Empty State */
          <div className="rounded-3xl border border-white/10 bg-[#0c140f] p-12 text-center max-w-xl mx-auto my-12">
            <div className="flex h-20 w-20 items-center justify-center rounded-3xl bg-white/5 border border-white/10 mx-auto mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-6">
                ...
                <div className="flex items-center justify-between text-xs mb-2">
                  {freeShippingRemaining > 0 ? (
                    <span className="text-white/80">
                      Add <strong className="text-[#FEF500]">${freeShippingRemaining.toFixed(2)}</strong> more to unlock <strong className="text-[#18C729]">FREE Shipping</strong>
                    </span>
                  ) : (
              ...
              {/* Items Card List */}
              <div className="rounded-3xl border border-white/10 bg-[#0c140f] overflow-hidden divide-y divide-white/5">
                {items.map((item) => (
                  <div key={item.id} className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 hover:bg-white/[0.02] transition-colors">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <Link
                          ...
                          className="text-base font-bold text-white hover:text-[#18C729] transition-colors block truncate"
                        >
                          {item.productName}
                        </Link>
                        <p className="text-xs font-mono text-white/40 mt-0.5">SKU: {item.sku}</p>
                        ...
                        {item.variantOptions && Object.keys(item.variantOptions).length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            {Object.entries(item.variantOptions).map(([k, v]) => (
                              <span
                                key={k}
                                className="rounded-md bg-black/40 border border-white/10 px-2 py-0.5 text-[11px] text-white/70"
                              >
                                {k}: {v}
                              </span>
                            ))}
                        ...
                        <p className="text-xs font-semibold text-white/80 mt-2 sm:hidden">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-white/5">
                      <div className="hidden sm:block text-right">
                        <p className="text-xs text-white/40 font-mono">${item.unitPrice.toFixed(2)}</p>
                        <p className="text-[10px] text-white/30">per unit</p>
                      </div>

                      {/* Quantity Selector */}
                      <div className="flex items-center rounded-xl border border-white/15 bg-black/30 p-1">
                        <button
                          type="button"
                          onClick={() => updateQuantity(item.id, item.quantity - 1)}
                          className="flex h-8 w-8 items-center justify-center text-white/70 hover:bg-white/10 hover:text-white rounded-lg transition-colors"
                        >
                          -
                        </button>
                      ...
                      <button
                        ...
                        className="text-white/40 hover:text-red-400 p-2 rounded-xl hover:bg-red-500/10 transition-colors"
              ...
              <div className="rounded-3xl border border-white/15 bg-[#0c140f] p-6 shadow-2xl space-y-5">
                ...
                <div className="space-y-3 text-xs">
                  ...
                  <div className="flex justify-between text-white/70">
                    ...
                    <span className="font-mono text-white/50">$0.00</span>
                  </div>

                  <div className="flex justify-between text-base font-extrabold text-white pt-4 border-t border-white/10">
                    <span>Total</span>
                    <span className="text-xl text-[#FEF500]">${total.toFixed(2)}</span>
                  </div>
                </div>
