...
export async function generateMetadata(): Promise<Metadata> {
  const [page, settings] = await Promise.all([getPageBySlug("privacy-policy"), getStoreSettings()]);
  const storeName = settings?.storeName || "ApexStore";
  const tpl = DEFAULT_PAGE_TEMPLATES["privacy-policy"];

  const title = page?.seoTitle || page?.title || tpl.title;
  const description = page?.seoDescription || tpl.seoDescription;
  const canonical = getAbsoluteUrl("/privacy-policy");
...
export default async function PrivacyPolicyPage(): Promise<React.JSX.Element> {
  ...
  const fallback = DEFAULT_PAGE_TEMPLATES["privacy-policy"];
