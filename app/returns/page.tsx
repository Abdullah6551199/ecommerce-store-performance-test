...
export async function generateMetadata(): Promise<Metadata> {
  const [page, settings] = await Promise.all([getPageBySlug("returns"), getStoreSettings()]);
  const storeName = settings?.storeName || "ApexStore";
  const tpl = DEFAULT_PAGE_TEMPLATES.returns;

  const title = page?.seoTitle || page?.title || tpl.title;
  const description = page?.seoDescription || tpl.seoDescription;
  const canonical = getAbsoluteUrl("/returns");
...
export default async function ReturnsPage(): Promise<React.JSX.Element> {
  const page = await getPageBySlug("returns");
  const fallback = DEFAULT_PAGE_TEMPLATES.returns;
