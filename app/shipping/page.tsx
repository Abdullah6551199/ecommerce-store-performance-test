...
export async function generateMetadata(): Promise<Metadata> {
  const [page, settings] = await Promise.all([getPageBySlug("shipping"), getStoreSettings()]);
  const storeName = settings?.storeName || "ApexStore";
  const tpl = DEFAULT_PAGE_TEMPLATES.shipping;

  const title = page?.seoTitle || page?.title || tpl.title;
  const description = page?.seoDescription || tpl.seoDescription;
  const canonical = getAbsoluteUrl("/shipping");
...
export default async function ShippingPage(): Promise<React.JSX.Element> {
  const page = await getPageBySlug("shipping");
  const fallback = DEFAULT_PAGE_TEMPLATES.shipping;
