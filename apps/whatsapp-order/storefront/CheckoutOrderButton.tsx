...
/**
 * "Order on WhatsApp" button for the Checkout page.
 * Appears directly below the "Place Order" button and matches its exact dimensions.
 * Stage 29.6: Persists order to D1 with source="whatsapp" before opening wa.me link.
 */
export default function CheckoutOrderButton(): React.JSX.Element | null {
  ...
  const handleOrderOnWhatsApp = async (e: React.MouseEvent) => {
    ...
    try {
      // Extract form data from checkout page DOM
      const nameInput = document.querySelector('input[name="customerName"]') as HTMLInputElement | null;
      ...
      const addressInput = document.querySelector('textarea[name="address"]') as HTMLTextAreaElement | null;
      const cityInput = document.querySelector('input[name="city"]') as HTMLInputElement | null;
      const notesInput = document.querySelector('input[name="notes"]') as HTMLInputElement | null;
      const postalCodeInput = document.querySelector('input[name="postalCode"]') as HTMLInputElement | null;

      // Saved info fallback from localStorage if available
      let savedInfo: WhatsAppCustomerInfo = {};
      try {
        const raw = localStorage.getItem("whatsapp_customer_info");
        if (raw) savedInfo = JSON.parse(raw);
      } catch {}

      const customerDetails: WhatsAppCustomerInfo = {
        name: nameInput?.value?.trim() || savedInfo.name || "Customer Name",
        phone: phoneInput?.value?.trim() || savedInfo.phone || phone,
        email: emailInput?.value?.trim() || savedInfo.email || "",
        address: addressInput?.value?.trim() || savedInfo.address || "Delivery Address",
        city: cityInput?.value?.trim() || savedInfo.city || "Karachi",
        postalCode: postalCodeInput?.value?.trim() || savedInfo.postalCode || "",
        notes: notesInput?.value?.trim() || savedInfo.notes || "",
      };

      // Persist customer details to localStorage for faster repeat orders
      try {
  ...
  return (
    <div data-app="whatsapp-order" className="w-full mt-3">
      <button
        ...
        id="whatsapp-checkout-order-btn"
        className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white py-4 text-sm font-extrabold shadow-xl shadow-emerald-500/20 transition-all cursor-pointer active:scale-[0.99] disabled:opacity-75"
      >
        <svg
