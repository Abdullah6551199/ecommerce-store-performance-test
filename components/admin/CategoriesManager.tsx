    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0c140f]/80 p-4 shadow-sm backdrop-blur-md">
          ...
          <div className="mt-2 text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">{stats.root}</div>
        </div>
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0c140f]/80 p-4 shadow-sm backdrop-blur-md">
          ...
          <div className="mt-2 text-2xl font-bold tracking-tight text-amber-600 dark:text-[#FEF500]">{stats.sub}</div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0c140f]/60 p-4 shadow-sm backdrop-blur-md sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="filter-status" className="text-xs text-zinc-600 dark:text-white/60">Status:</label>
          <select
            id="filter-status"
            value={statusFilter}
      ...
      {/* Table Container */}
      <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0c140f]/80 backdrop-blur-md shadow-sm dark:shadow-xl">
        {isLoading && categories.length === 0 ? (
          ...
          <div className="p-16 text-center">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-zinc-100 dark:bg-white/5 text-amber-500 dark:text-[#FEF500]">
              <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
              </svg>
            </div>
            <h3 className="mt-4 text-sm font-semibold text-zinc-900 dark:text-white">No categories found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.02] text-zinc-600 dark:text-white/60">
                    ...
                    >
                      ...
                      <td className="py-3.5 pl-6 pr-4">
                        ...
                        >
                          ...
                          {cat.depth > 0 && (
                            <span className="text-amber-500 dark:text-[#FEF500]/60 font-mono text-sm shrink-0 select-none">
                              ↳
                            </span>
                          )}

                          {/* Thumbnail */}
                          <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-white/5">
                            {cat.imageUrl ? (
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-zinc-900 dark:text-white group-hover:text-[#18C729] transition-colors truncate">
                                {cat.name}
                              </span>
                              {cat.depth === 0 && (
                                <span className="rounded-md bg-zinc-100 dark:bg-white/5 px-1.5 py-0.5 text-[9px] font-mono text-zinc-600 dark:text-white/50 border border-zinc-200 dark:border-white/10">
                                  Root
                                </span>
                              )}
                            </div>
                            {cat.description && (
                              <p className="text-[11px] text-zinc-500 dark:text-white/40 truncate max-w-xs">
                                {cat.description}
                              </p>
                            )}
                          </div>
                      ...
                      {/* Slug */}
                      <td className="px-4 py-3.5 font-mono text-zinc-600 dark:text-white/60">
                        <span className="rounded-lg bg-zinc-100 dark:bg-black/40 px-2 py-1 border border-zinc-200 dark:border-white/10 text-[11px] text-zinc-800 dark:text-zinc-200">
                          /{cat.slug}
                        </span>
                      </td>

                      {/* Parent Category */}
                      <td className="px-4 py-3.5 text-zinc-700 dark:text-white/70">
                        {hasParent ? (
                          <span className="inline-flex items-center gap-1.5 rounded-lg border border-amber-500/20 dark:border-[#FEF500]/20 bg-amber-50 dark:bg-[#FEF500]/5 px-2 py-1 text-[11px] text-amber-700 dark:text-[#FEF500]">
                          ...
                          <span className="text-zinc-400 dark:text-white/30 text-[11px]">—</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        {cat.status === "active" ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-300 dark:border-white/15 bg-zinc-100 dark:bg-white/5 px-2.5 py-0.5 text-[10px] font-medium text-zinc-600 dark:text-white/50">
                      ...
                      <td className="py-3.5 pl-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            title="Edit Category"
                            className="rounded-lg border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-white/5 p-1.5 text-zinc-700 dark:text-white/70 hover:border-[#18C729]/40 hover:bg-[#18C729]/10 hover:text-[#18C729] transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="rounded-lg border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-white/5 p-1.5 text-zinc-700 dark:text-white/70 hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-500 dark:hover:text-red-400 transition-colors"
                          >
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {categoryToDelete && (
        ...
        >
          <div className="w-full max-w-md rounded-3xl border border-zinc-200 dark:border-white/15 bg-white dark:bg-[#0a110c] p-6 shadow-2xl">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-red-500/30 bg-red-500/10 text-red-500 dark:text-red-400">
              <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            </div>
            <h3 className="mt-4 text-base font-bold text-zinc-900 dark:text-white">Delete Category</h3>
            <p className="mt-1.5 text-xs text-zinc-600 dark:text-white/60">
              Are you sure you want to delete <span className="font-semibold text-zinc-900 dark:text-white">&quot;{categoryToDelete.name}&quot;</span>?
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button
                ...
                disabled={isDeleting}
                className="rounded-xl border border-zinc-300 dark:border-white/10 bg-zinc-100 dark:bg-white/5 px-4 py-2 text-xs font-medium text-zinc-700 dark:text-white/70 hover:bg-zinc-200 dark:hover:bg-white/10"
              >
                Cancel
              </button>
              <button
