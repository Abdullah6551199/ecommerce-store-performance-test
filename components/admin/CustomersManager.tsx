      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-[#0c140f] p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-white/50">Total Customers</span>
          ...
          <div className="mt-3 text-2xl font-extrabold text-white">{summary.totalCustomers}</div>
          <p className="mt-1 text-[11px] text-white/40">Unique purchasing profiles</p>
        </div>

        <div className="rounded-2xl border border-white/10 bg-[#0c140f] p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-white/50">Total Orders Placed</span>
          ...
          <div className="mt-3 text-2xl font-extrabold text-white">{summary.totalOrders}</div>
          <p className="mt-1 text-[11px] text-white/40">Across all customers</p>
        </div>

        <div className="rounded-2xl border border-white/10 bg-[#0c140f] p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-white/50">Lifetime Revenue</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#18C729]/15 text-[#18C729]">
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="mt-3 text-2xl font-extrabold text-white">${summary.totalRevenue.toFixed(2)}</div>
          <p className="mt-1 text-[11px] text-white/40">Cumulative gross order volume</p>
        </div>

        <div className="rounded-2xl border border-white/10 bg-[#0c140f] p-4 shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-white/50">Avg Order Value</span>
          ...
          <div className="mt-3 text-2xl font-extrabold text-white">${summary.averageOrderValue.toFixed(2)}</div>
          <p className="mt-1 text-[11px] text-white/40">Per customer transaction</p>
        </div>
      </div>

      {/* Search Bar */}
      <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-[#0c140f] p-3 shadow-lg">
        ...
        {search && (
          <button
            type="button"
            onClick={() => setSearch("")}
            className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/60 hover:text-white transition-colors"
          >
            Clear Search
          </button>
        )}
      ...
      {error && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-xs text-red-400 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => fetchCustomers(search)}
            className="underline hover:text-red-300 ml-3"
          >
            Retry
          </button>
        </div>
              ...
              {isLoading && customers.length === 0 ? (
                ...
                <tr>
                  <td colSpan={7} className="p-12 text-center text-white/40">
                    ...
                    <p className="text-sm font-medium text-white/70">No customers found</p>
                  ...
                  <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#18C729]/20 to-[#FEF500]/20 font-mono text-xs font-bold text-[#18C729] border border-white/10">
                          {getInitials(c.name)}
                        </div>
                        <div>
                          <div className="font-semibold text-white">{c.name}</div>
                          <div className="text-[10px] font-mono text-white/40">Customer #{c.id.slice(0, 8)}</div>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="space-y-1">
                        {c.email ? (
                          <div className="flex items-center gap-1.5">
                            <span className="truncate max-w-[160px] text-white/90">{c.email}</span>
                            <button
                              type="button"
                          ...
                          <span className="text-white/30 italic">No email provided</span>
                        )}
                        {c.phone && (
                    ...
                    <td className="px-5 py-4">
                      <div>
                        ...
                        <div className="truncate max-w-[180px] text-[11px] text-white/40">{c.address || "—"}</div>
                      </div>
                    </td>

                    {/* Total Orders */}
                    <td className="px-5 py-4 text-center">
                      <span className="inline-flex items-center justify-center rounded-full bg-white/10 px-2.5 py-0.5 text-xs font-semibold text-white">
                        {c.totalOrders}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="font-mono font-bold text-[#18C729]">
                    ...
                    <td className="px-5 py-4 text-right">
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-medium text-white/80 hover:border-[#18C729]/40 hover:bg-[#18C729]/10 hover:text-[#18C729] transition-all"
                      >
                        <span>Orders</span>
                        <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
