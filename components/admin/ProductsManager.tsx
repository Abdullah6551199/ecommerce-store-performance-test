    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-white/60">
            Manage SKU inventory, pricing tiers, Cloudflare R2 gallery assets, and store listings.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#18C729] to-[#12a822] px-4 py-2.5 text-xs font-semibold text-black shadow-lg shadow-[#18C729]/20 hover:brightness-110 active:scale-95 transition-all"
          >
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0c140f]/80 p-4 backdrop-blur-md">
          ...
          <div className="mt-2 text-2xl font-bold tracking-tight text-[#FEF500]">{stats.lowOrOut}</div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0c140f]/80 p-4 backdrop-blur-md">
          ...
          <div className="mt-2 text-2xl font-bold tracking-tight text-white">{stats.totalUnits}</div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0c140f]/80 backdrop-blur-md shadow-xl">
        {isLoading && productsList.length === 0 ? (
          <div className="p-12 text-center text-xs text-white/50">
            ...
            Loading catalog from Cloudflare D1...
          </div>
        ) : error ? (
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="mt-4 text-sm font-semibold text-white">No products found</h3>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-white/5">
                {paginatedProducts.map((prod) => {
                  ...
                  return (
                    <tr key={prod.id} className="group transition-colors hover:bg-white/[0.02]">
                      ...
                      <td className="py-3.5 pl-6 pr-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <span className="font-semibold text-white group-hover:text-[#18C729] transition-colors truncate block">
                              {prod.name}
                            </span>
                      ...
                      <td className="px-4 py-3.5 font-mono text-white/70">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {prod.variants && prod.variants.length > 0 && (
                            <span className="rounded-md bg-[#FEF500]/10 border border-[#FEF500]/30 px-1.5 py-0.5 text-[9px] font-sans font-bold text-[#FEF500]">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5">
                          <span className="font-mono text-white/90">{prod.stockQuantity}</span>
                          {isOutOfStock ? (
                            <span className="rounded px-1.5 py-0.5 text-[9px] font-semibold bg-red-500/10 text-red-400 border border-red-500/20">
                              Out
                            </span>
                          ) : isLowStock ? (
                            <span className="rounded px-1.5 py-0.5 text-[9px] font-semibold bg-[#FEF500]/10 text-[#FEF500] border border-[#FEF500]/20">
                              Low
                            </span>
                          ) : (
                            <span className="rounded px-1.5 py-0.5 text-[9px] font-semibold bg-[#18C729]/10 text-[#18C729] border border-[#18C729]/20">
                              In Stock
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-4 py-3.5">
                        {prod.categoryName ? (
                          <span className="rounded-lg bg-white/5 px-2 py-1 text-[11px] text-white/70 border border-white/10">
                            {prod.categoryName}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3.5">
                        {prod.status === "published" ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-2.5 py-0.5 text-[10px] font-medium text-white/60">
                            <span className="h-1.5 w-1.5 rounded-full bg-white/40" />
                            Draft
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-[10px] font-medium text-white/40">
                            Archived
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-3.5 pl-4 pr-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="rounded-lg border border-white/10 bg-white/5 p-1.5 text-white/70 hover:border-[#FEF500]/40 hover:bg-[#FEF500]/10 hover:text-[#FEF500] transition-colors disabled:opacity-50"
                          >
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {productToDelete && (
        ...
        >
          <div className="w-full max-w-md rounded-3xl border border-white/15 bg-[#0a110c] p-6 shadow-2xl">
            ...
            <h3 className="mt-4 text-base font-bold text-white">Delete Product</h3>
            <p className="mt-1.5 text-xs text-white/60">
              Are you sure you want to permanently delete <span className="font-semibold text-white">&quot;{productToDelete.name}&quot;</span> (SKU: {productToDelete.sku})? All associated gallery records will also be removed.
            </p>
