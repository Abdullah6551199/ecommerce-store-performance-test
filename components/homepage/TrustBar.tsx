...
export default function TrustBar({ items = DEFAULT_TRUST_ITEMS }: TrustBarProps): React.JSX.Element {
  ...
  const renderIcon = (icon: TrustItem["icon"]) => {
    switch (icon) {
      case "shipping":
        return (
          <svg className="h-6 w-6 text-[#960DF2] dark:text-[#C06EF7]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {activeItems.map((item, idx) => (
          <div key={idx} className="flex items-start gap-3.5">
            ...
            <div className="space-y-0.5">
              <h4 className="text-sm font-black text-[#3C0561] dark:text-white leading-tight">
                {item.title}
              </h4>
              <p className="text-xs text-[#5A0891]/80 dark:text-purple-200/70 leading-relaxed font-medium">
                {item.description}
              </p>
            </div>
          </div>
