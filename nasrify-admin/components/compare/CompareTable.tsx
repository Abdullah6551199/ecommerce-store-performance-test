...
export default function CompareTable(): React.JSX.Element {
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center space-y-4">
        <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-[#25D366] border-r-transparent" />
        <p className="text-sm font-semibold text-[#25D366] dark:text-zinc-400">
  ...
  if (products.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center space-y-6">
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-[#DCFCE7] dark:bg-[#18181B]/40 text-[#25D366] dark:text-[#1EA855]">
          <svg className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
          </svg>
        </div>
        <div className="space-y-2">
          <h1 className="text-2xl sm:text-3xl font-black text-[#18181B] dark:text-white">
            No Products Selected to Compare
          </h1>
          <p className="text-sm text-[#1EA855]/80 dark:text-zinc-300/70">
        ...
        <div className="pt-2">
          <Link
            href="/shop"
            className="inline-flex items-center gap-2 rounded-xl bg-[#25D366] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-[#25D366]/20 hover:bg-[#1EA855] transition-all"
          >
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      {/* Top Header & Actions */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#E4E4E7]/70 dark:border-zinc-800/50 pb-6">
        <div>
        ...
        <div className="flex items-center gap-3">
          {products.length < MAX_COMPARE_LIMIT && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-[#E4E4E7] dark:border-zinc-700 bg-white dark:bg-[#18181B] px-3.5 py-2 text-xs font-semibold text-[#1EA855] dark:text-zinc-300 hover:border-[#25D366] shadow-sm"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="overflow-x-auto rounded-2xl border border-[#E4E4E7]/80 dark:border-zinc-800/60 bg-white dark:bg-[#18181B]/90 shadow-xl backdrop-blur-md">
        <table className="w-full min-w-[650px] border-collapse text-left text-xs">
          <thead>
            {/* Header: Images & Names & Remove */}
            <tr className="border-b border-[#E4E4E7]/60 dark:border-zinc-800/40 bg-[#F4F4F5]/50 dark:bg-[#18181B]/40">
              <th className="w-48 p-4 font-bold text-[#18181B] dark:text-zinc-300 uppercase tracking-wider">
                Product
              </th>
              {products.map((p) => {
                ...
                return (
                  <th key={p.id} className="p-4 align-top w-64">
                    <div className="space-y-3">
                      <div className="relative aspect-square w-full max-w-[180px] mx-auto overflow-hidden rounded-xl border border-[#E4E4E7] dark:border-zinc-800/60 bg-[#F4F4F5]/40 dark:bg-[#18181B]/40">
                        {img ? (
                          <Image
                            src={img}
                            alt={p.name}
                      ...
                      <div className="text-center space-y-1">
                        <Link
                          href={`/product/${p.slug}`}
                          className="font-bold text-sm text-[#18181B] dark:text-white hover:text-[#25D366] transition line-clamp-2"
                        >
                          {p.name}
                        </Link>
                        <p className="text-[11px] text-zinc-400 font-medium">{p.brand}</p>
                      </div>
                    </div>
                  </th>
                );
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 dark:divide-zinc-200 dark:divide-zinc-800 text-[#18181B] dark:text-zinc-200">
            {/* Row: Price & Differences */}
            <tr>
              <td className="p-4 font-bold bg-[#F4F4F5]/20 dark:bg-[#18181B]/20 text-[#18181B] dark:text-zinc-300">
                Price
              </td>
              {products.map((p) => {
              ...
              {products.map((p) => (
                <td key={`rating-${p.id}`} className="p-4 align-middle">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] text-zinc-400">({p.reviewCount})</span>
                  </div>
                </td>
              ))}
            </tr>
            ...
            <tr>
              ...
              {products.map((p) => (
                <td key={`desc-${p.id}`} className="p-4 align-top text-[11px] leading-relaxed text-zinc-600 dark:text-zinc-300/80">
            ...
            {/* Row: Add to Cart */}
            <tr className="bg-[#F4F4F5]/30 dark:bg-[#18181B]/30">
              <td className="p-4 font-bold text-[#18181B] dark:text-zinc-300">
                Action
              </td>
              {products.map((p) => {
                ...
                return (
                  <td key={`action-${p.id}`} className="p-4 align-middle">
                    <button
                      ...
                      className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#25D366] hover:bg-[#1EA855] px-4 py-2.5 text-xs font-bold text-white shadow-md shadow-[#25D366]/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                    >
                      {addingId === p.id ? (
                        <span>Adding...</span>
                      ) : isOutOfStock ? (
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-lg rounded-3xl border border-[#E4E4E7] dark:border-zinc-800 bg-white dark:bg-[#18181B] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[#E4E4E7]/60 dark:border-zinc-800/40 pb-3">
              <h2 className="text-base font-bold text-[#18181B] dark:text-white">
                Add Product to Compare
              </h2>
              <button
                type="button"
                onClick={() => setIsSearchOpen(false)}
                className="text-zinc-400 hover:text-[#25D366] text-lg font-bold"
              >
                ×
              </button>
            </div>
            ...
            <div className="relative">
              <input
                ...
                autoFocus
                className="w-full rounded-xl border border-[#E4E4E7] dark:border-zinc-700 bg-[#F4F4F5]/50 dark:bg-[#18181B]/50 px-4 py-2.5 text-xs text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-[#25D366]"
              />
              {isSearching && (
                <div className="absolute right-3 top-3 h-4 w-4 animate-spin rounded-full border-2 border-[#25D366] border-r-transparent" />
              )}
            </div>

            <div className="max-h-72 overflow-y-auto space-y-2 divide-y divide-zinc-200 dark:divide-zinc-800 dark:divide-zinc-200 dark:divide-zinc-800">
              ...
              {searchResults.map((p) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-lg bg-[#F4F4F5]">
                        {p.mainImage && (
                          <Image
                      ...
                      <div>
                        <p className="text-xs font-bold text-[#18181B] dark:text-white line-clamp-1">
                          {p.name}
                        </p>
                        <p className="text-[11px] font-semibold text-[#25D366] dark:text-[#1EA855]">
                    ...
                    <button
                      ...
                      className={`rounded-xl px-3 py-1.5 text-xs font-bold transition ${
                        isSelected
                          ? "bg-[#DCFCE7] dark:bg-[#18181B]/40 text-zinc-400 cursor-not-allowed"
                          : "bg-[#25D366] hover:bg-[#1EA855] text-white cursor-pointer"
                      }`}
                    >
