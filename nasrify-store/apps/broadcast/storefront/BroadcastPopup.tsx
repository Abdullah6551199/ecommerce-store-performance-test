...
import type { BroadcastStorefrontData, BroadcastAppSettings } from "../shared/types";

export default function BroadcastPopup(): React.JSX.Element | null {
  const [broadcast, setBroadcast] = useState<BroadcastStorefrontData | null>(null);
  const [settings, setSettings] = useState<Partial<BroadcastAppSettings> | null>(null);
  ...
  const fetchActiveBroadcast = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as {
        success?: boolean;
        broadcast?: BroadcastStorefrontData | null;
        settings?: Partial<BroadcastAppSettings> | null;
      };
  ...
  }, [fetchActiveBroadcast, isAdminRoute, settings?.popupDelaySeconds]);
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case "promotion":
        return {
          label: "Special Offer",
          bgColor: "bg-[#25D366]/10 text-[#25D366] border-[#25D366]/20",
          icon: (
  ...
  const positionClass =
    settings?.popupPosition === "bottom"
      ? "items-end pb-8 sm:pb-12"
      : settings?.popupPosition === "center"
      ? "items-center"
      : "items-start pt-8 sm:pt-14";

  return (
    <div
      data-app="broadcast"
      className={`fixed inset-0 z-[999] flex justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 ${positionClass}`}
      role="dialog"
      aria-modal="true"
    >
      <div className="relative w-full max-w-md rounded-3xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl transform transition-all animate-in zoom-in-95 duration-200">
        <div className="h-1.5 w-full bg-gradient-to-r from-[#1EA855] to-[#25D366]" />
        ...
        <button
          ...
          className="absolute top-4 right-4 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-zinc-100 dark:bg-white/10 text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition shadow-sm cursor-pointer"
        >
        ...
        <div className="p-6 sm:p-7 space-y-4">
          ...
          {(broadcast.buttonText || broadcast.linkUrl) && (
            <button
              ...
              className="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-[#1EA855] to-[#25D366] hover:from-[#188e47] hover:to-[#1EA855] text-white font-extrabold text-sm shadow-lg shadow-emerald-500/20 transition duration-150 flex items-center justify-center gap-2 active:scale-[0.98] cursor-pointer"
            >
          ...
          <div className="pt-2 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <button
              type="button"
              onClick={handleClose}
              className="text-xs font-semibold text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 underline transition cursor-pointer"
            >
              Dismiss
            </button>
          </div>
