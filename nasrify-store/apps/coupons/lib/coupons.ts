...
import type {
  CartItemValidationInput,
  CouponValidationResult,
  CouponStats,
} from "../shared/types";

export type { CouponRecord, NewCouponRecord, CouponUsageRecord };
export type { CartItemValidationInput, CouponValidationResult, CouponStats };
...
// Rate limiting cache: max 10 coupon checks per minute per IP
const validationRateLimits = new Map<string, { count: number; resetAt: number }>();

export function checkValidationRateLimit(ipOrKey: string): boolean {
  const now = Date.now();
  const entry = validationRateLimits.get(ipOrKey);
  if (!entry || now > entry.resetAt) {
    validationRateLimits.set(ipOrKey, { count: 1, resetAt: now + 60000 });
    return true;
  }
  if (entry.count >= 10) {
    return false;
...
const COUPONS_CACHE_TTL_MS = 20000; // 20s micro-cache per permanent rules
 ...
 * Deduplicated via React.cache with 20s in-memory TTL
 */
...
/**
 * Get coupon stats for admin widget & dashboard
 */
export async function getCouponStats(): Promise<CouponStats> {
  const db = getDb();
  if (db) {
    const allCoupons = await db
      .select({ id: coupons.id, isActive: coupons.isActive, code: coupons.code, usedCount: coupons.usedCount })
      .from(coupons)
      .limit(500);
    ...
    let mostUsedCoupon = { code: "None", count: 0 };
    if (allCoupons.length > 0) {
      const sorted = [...allCoupons].sort((a, b) => (b.usedCount || 0) - (a.usedCount || 0));
      if (sorted[0] && sorted[0].usedCount > 0) {
        mostUsedCoupon = { code: sorted[0].code, count: sorted[0].usedCount };
      }
    }
  ...
  const activeCount = memoryCoupons.filter((c) => c.isActive).length;
  let topCoup = { code: "None", count: 0 };
  const sorted = [...memoryCoupons].sort((a, b) => b.usedCount - a.usedCount);
  if (sorted[0] && sorted[0].usedCount > 0) {
    topCoup = { code: sorted[0].code, count: sorted[0].usedCount };
  }
